...
class GoogleRobotDefaultConfig:
    def __init__(self, mobile_base=False, base_arm_drive_mode='force') -> None:
        ...
        if self.base_arm_drive_mode == 'acceleration':
            raise NotImplementedError('PD parameters not yet tuned for acceleration drive mode.')
        elif self.base_arm_drive_mode == 'force':
            # arm_pd_ee_delta_pose_align_interpolate_by_planner
            self.arm_stiffness = [1700.0, 1737.0471680861954, 979.975871856535, 930.0, 1212.154500274304, 432.96500923932535, 468.0013365498738, 2000, 2000]
            self.arm_damping = [1059.9791902443303, 1010.4720585373592, 767.2803161582076, 680.0, 674.9946964336588, 274.613381336198, 340.532560578637, 900, 900]
    ...
    def cameras(self):
        return CameraConfig(
            uid="overhead_camera",
            p=[0, 0, 0],
            q=[0.5, 0.5, -0.5, 0.5], # SAPIEN uses ros camera convention; the rotation matrix of link_camera's pose is in opencv convention, so we need to transform it to ros convention
            width=640,
            height=512,
